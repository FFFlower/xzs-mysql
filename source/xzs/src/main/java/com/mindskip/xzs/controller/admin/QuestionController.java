...
import com.mindskip.xzs.viewmodel.admin.question.QuestionDelRequestVM;
...
public class QuestionController extends BaseApiController {
    ...
    public RestResponse<PageInfo<QuestionResponseVM>> pageList(@RequestBody QuestionPageRequestVM model) {
        PageInfo<Question> pageInfo = questionService.pageByContent(model);
    ...
    public RestResponse edit(@RequestBody @Valid QuestionEditRequestVM model) {
        ...
        if (null == model.getId()) {
            ...
            questionService.addEsData(model);
        } else {
            questionService.updateFullQuestion(model);
            questionService.updateEsData(model);
        }
    ...
    @RequestMapping(value = "/first/{levelId}/{subjectId}", method = RequestMethod.POST)
    ...
    public RestResponse delete(@PathVariable Integer id) {
        ...
        ElasticsearchUtil.deleteDataById("exam-system", "question", id.toString());
        return RestResponse.ok();
    }
    ...
    public RestResponse delete(@RequestBody Integer[] ids) {
        if (null == ids || ids.length <= 0) {
            return RestResponse.fail(SystemCode
                    .ParameterValidError.getCode(), SystemCode
                    .ParameterValidError.getMessage());
        }
        questionService.batchDelete(ids);
        return RestResponse.ok();
    }

    @RequestMapping(value = "/delByCondition", method = RequestMethod.POST)
    public RestResponse delByCondition(@RequestBody QuestionDelRequestVM requestVM) {
        if (null == requestVM.getSubjectId()) {
            return RestResponse.fail(SystemCode
                    .ParameterValidError.getCode(), SystemCode
                    .ParameterValidError.getMessage());
        }
        questionService.delByCondition(requestVM.getLevel(), requestVM.getSubjectId());
        return RestResponse.ok();
    }

    @RequestMapping(value = "/importExcel", method = RequestMethod.POST)
    public RestResponse importExcel(@RequestParam("level") Integer level,@RequestParam("subjectId") Integer subjectId, @RequestParam("file") MultipartFile file) {
        try {
            questionService.readExcel(file, level, subjectId, getCurrentUser().getId());
