...
public class ElasticsearchUtil {
    ...
    public static boolean createIkMapping(String index,String type,String propertiesName) throws IOException {
        //创建映射
        XContentBuilder mapping = XContentFactory.jsonBuilder()
                .startObject()
                    .startObject("properties")
                        .startObject(propertiesName)
                            .field("type","text")
                ...
                .endObject();
        //创建索引
        AcknowledgedResponse response = client.admin().indices().prepareCreate(index).addMapping(type,mapping).get();
        //pois：索引名   cxyword：类型名（可以自己定义）
//        PutMappingRequest putmap = Requests.putMappingRequest(index).type(type).source(mapping);
//        //为索引添加映射
//        AcknowledgedResponse response1 = client.admin().indices().putMapping(putmap).actionGet();
    ...
    public static EsPage searchDataPage(String index, String type, int startPage, int pageSize, QueryBuilder query, String fields, String sortField, String highlightField) {
        ...
        if (searchResponse.status().getStatus() == 200) {
            ...
            return new EsPage(startPage, pageSize, totalHits < 100 ? (int) totalHits:100, sourceList);
        }
